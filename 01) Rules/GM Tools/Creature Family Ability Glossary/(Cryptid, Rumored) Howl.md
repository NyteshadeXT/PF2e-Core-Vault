---
obsidianUIMode: preview
title: "(Cryptid, Rumored) Howl"
cssclasses: pf2e,pf2e-action
tags:
  - trait/auditory
  - trait/emotion
  - trait/fear
  - trait/mental
  - trait/common
  - category/offensive
aliases: "Compendium.pf2e.bestiary-family-ability-glossary.Item.KLMdplDgOfXSLh6g"
license: OGL
---
# (Cryptid, Rumored) Howl `pf2:2`

### [[auditory]][[emotion]][[fear]][[Mental]]

**Category** offensive; 




The rumored cryptid calls out with a wretched sound no living creature should make. Each creature in a 30-foot emanation must attempt a Will check save. A creature that fails is [[Conditions/Frightened 1|Frightened 1]] (or [[Conditions/Frightened 1|Frightened 2]] on a critical failure). A creature that succeeds is temporarily immune for 1 minute. The DC is the high DC for a creature of the rumored cryptid's level.

*Source: Pathfinder Dark Archive*
*OGL*