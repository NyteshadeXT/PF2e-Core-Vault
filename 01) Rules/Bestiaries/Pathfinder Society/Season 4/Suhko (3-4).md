---
title: "Suhko (3-4)"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.pfs-season-4-bestiary.Actor.7W9dzIWzgEOB2UTK" 
tags:
  - pf2e/creature/type/changeling
  - pf2e/creature/type/chaotic
  - pf2e/creature/type/evil
  - pf2e/creature/type/human
  - pf2e/creature/type/humanoid
  - pf2e/creature/level/3
statblock: inline
name: "Suhko (3-4)"
level: 3
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder Society Scenario #4-13: Within the Prairies"
name: "Suhko (3-4)"
level: "Creature 3"
rare_03: "Unique"
alignment: ""
size: "Medium"
trait_01: "changeling"
trait_02: "chaotic"
trait_03: "evil"
trait_04: "human"
trait_05: "humanoid"
modifier: 8
perception:
  - name: "Perception"
    desc: "+8; Darkvision"
languages: "Aklo, Chthonian, Common, Draconic, Sakvroth"
skills:
  - name: "Skills"
    desc: "Deception: +8, Intimidation: +8, Occultism: +10, Stealth: +6"
abilityMods: [0, 0, 2, 4, 2, 2]
speed: 25 feet
sourcebook: "_Pathfinder Society Scenario #4-13: Within the Prairies_"
ac: 16
armorclass:
  - name: AC
    desc: "16; __Fort__ +8, __Ref__ +6, __Will__ +10; +2 circumstance to all saves vs. dream and sleep"
hp: 40
health:
  - name: ""
  - name: HP
    desc: "40"
abilities_top:
  - name: ""
  - name: "Items"
    desc: "[[Equipment/Sling|Sling]], [[Equipment/Staff|Staff]], [[Equipment/Explorer's Clothing|Explorer's Clothing]], 10x [[Equipment/Sling Bullets|Sling Bullets]]"
abilities_mid:
  - name: ""
attacks:
  - name: ""

  - name: "Melee"
    desc: "`pf2:1` Living Hair +6 (agile, disarm, finesse, trip, unarmed)\n__Damage__  1d4 bludgeoning"

  - name: "Melee"
    desc: "`pf2:1` Staff +6 (two-hand d8)\n__Damage__  1d4 bludgeoning"

  - name: "Ranged"
    desc: "`pf2:1` Sling +6 (propulsive, range increment 50 feet)\n__Damage__  1d6 bludgeoning"

  - name: "Occult Prepared Spells"
    desc: "DC 20, attack +12; __2nd __  _[[Spells/Blood Vendetta|Blood Vendetta]]_, _[[Spells/Vomit Swarm|Vomit Swarm]]_; __1st __  _[[Spells/Déjà Vu|Déjà Vu]]_, _[[Spells/Grim Tendrils|Grim Tendrils]]_, _[[Spells/Phantom Pain|Phantom Pain]]_\n__Cantrips__  __(2nd)__ _[[Spells/Void Warp|Chill Touch]]_, _[[Spells/Daze|Daze]]_, _[[Spells/Guidance|Guidance]]_, _[[Spells/Telekinetic Hand|Mage Hand]]_, _[[Spells/Shield|Shield]]_"

  - name: "Familiar (Runt)"
    desc: "  Suhko has a Tiny prairie drake named Runt as a familiar. Runt has a speed of 25 feet, a burrow speed of 20 feet, 15 hit points, an AC of 16, and the following additional familiar abilities:[[Familiar Abilities/Restorative Familiar|Restorative Familiar]]and speech (Common). Note that as normal for a minion, Suhko must spend 1 action to command Runt to give him the 2 actions needed to use Restorative Familiar."

  - name: "Restorative Familiar"
    desc: "`pf2:2` (concentrate) **Frequency** once per day\n**Effect** Runt gives up some of his animating energy and heals Suhko. He must be in Suhko’s space to do so. Runt restores 2d8 Hit Points to Suhko.\n**Note** Suhko must spend 1 action to command Runt to give him the 2 actions needed to use Restorative Familiar."
 
```

```encounter-table
name: Suhko (3-4)
creatures:
  - 1: Suhko (3-4)
```



