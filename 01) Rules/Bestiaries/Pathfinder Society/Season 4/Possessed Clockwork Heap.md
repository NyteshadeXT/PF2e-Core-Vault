---
title: "Possessed Clockwork Heap"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.pfs-season-4-bestiary.Actor.c3YWo3zyPsybTYhC" 
tags:
  - pf2e/creature/type/chaotic
  - pf2e/creature/type/clockwork
  - pf2e/creature/type/construct
  - pf2e/creature/type/evil
  - pf2e/creature/level/7
statblock: inline
name: "Possessed Clockwork Heap"
level: 7
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder Society Scenario #4-06: Signal from the Electric Laboratory"
name: "Possessed Clockwork Heap"
level: "Creature 7"
rare_03: "Rare"
alignment: ""
size: "Medium"
trait_01: "chaotic"
trait_02: "clockwork"
trait_03: "construct"
trait_04: "evil"
modifier: 14
perception:
  - name: "Perception"
    desc: "+14; Darkvision"
skills:
  - name: "Skills"
    desc: "Athletics: +17, Stealth: +15"
abilityMods: [6, 4, 5, -2, 3, 3]
speed: 20 feet
sourcebook: "_Pathfinder Society Scenario #4-06: Signal from the Electric Laboratory_"
ac: 25
armorclass:
  - name: AC
    desc: "25; __Fort__ +18, __Ref__ +15, __Will__ +12"
hp: 95
health:
  - name: ""
  - name: HP
    desc: "95; __Immunities__  mental,  bleed,  death effects,  disease,  doomed,  drained,  fatigued,  healing,  nonlethal attacks,  paralyzed,  poison,  sickened,  spirit,  unconscious,  vitality,  void; __Weaknesses__ electricity 5, orichalcum 5; __Resistances__ physical 5 (except adamantine or orichalcum)"
abilities_top:
  - name: ""

abilities_mid:
  - name: ""
  - name: "Overburden"
    desc: "`pf2:2`  **Requirements** The infested clockwork heap's last action was a success with an infested touch Strike\n**Effect** A fragment of the infesting spirit attaches to the target's clothing or armor, making the creature encumbered. The encumbered condition from Overburden disappears when the infested clockwork is destroyed or after 10 minutes, whichever comes first."

attacks:
  - name: ""

  - name: "Melee"
    desc: "`pf2:1` Infested Touch +18 (finesse, magical)\n__Damage__  2d12 + 6 void"

  - name: "Ranged"
    desc: "`pf2:1` Wrathful Toss (bludgeoning) +16 (magical, occult, range increment 60 feet)\n__Damage__  3d8 bludgeoning"

  - name: "Ranged"
    desc: "`pf2:1` Wrathful Toss (piercing) +16 (magical, occult, range increment 160 feet)\n__Damage__  3d8 piercing"

  - name: "Ranged"
    desc: "`pf2:1` Wrathful Toss (slashing) +16 (magical, occult, range increment 60 feet)\n__Damage__  3d8 slashing"

  - name: "Scrap Tempest"
    desc: "`pf2:2` (concentrate,occult) The infested clockwork heap unleashes a cloud of loosely connected debris that whirls around its body at incredible speed. Each creature within 30 feet takes 3d8 bludgeoning damage (DC 25 Fortitude check)."
 
```

```encounter-table
name: Possessed Clockwork Heap
creatures:
  - 1: Possessed Clockwork Heap
```



