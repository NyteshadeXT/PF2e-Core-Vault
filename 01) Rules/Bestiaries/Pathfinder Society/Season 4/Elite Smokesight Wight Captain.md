---
title: "Elite Smokesight Wight Captain"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.pfs-season-4-bestiary.Actor.BRo5HCigb4YTjJys" 
tags:
  - pf2e/creature/type/evil
  - pf2e/creature/type/lawful
  - pf2e/creature/type/undead
  - pf2e/creature/type/unholy
  - pf2e/creature/type/wight
  - pf2e/creature/level/11
statblock: inline
name: "Elite Smokesight Wight Captain"
level: 11
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder Society Scenario #4-15: In Glorious Battle"
name: "Elite Smokesight Wight Captain"
level: "Creature 11"
rare_03: "Rare"
alignment: ""
size: "Medium"
trait_01: "evil"
trait_02: "lawful"
trait_03: "undead"
trait_04: "unholy"
trait_05: "wight"
modifier: 21
perception:
  - name: "Perception"
    desc: "+21; Darkvision"
languages: "Common, Necril"
skills:
  - name: "Skills"
    desc: "Athletics: +23, Intimidation: +24, Warfare Lore: +24"
abilityMods: [6, 3, 4, 5, 3, 3]
speed: 25 feet
sourcebook: "_Pathfinder Society Scenario #4-15: In Glorious Battle_"
ac: 31
armorclass:
  - name: AC
    desc: "31 (33 with shield raised); __Fort__ +23, __Ref__ +18, __Will__ +20"
hp: 200
health:
  - name: ""
  - name: HP
    desc: "200, void healing; __Immunities__  death effects,  disease,  paralyzed,  poison,  unconscious"
abilities_top:
  - name: ""
  - name: "Items"
    desc: "[[Equipment/Longsword|+1 Striking Longsword]], [[Equipment/Steel Shield|Adamantine Shield]], [[Equipment/Full Plate|Full Plate]]"
  - name: "[[Actor.pomHkkLGJxTvpAyA.Item.TOApogIgln0yhGda|Smoke Vision]]"
    desc: "  The wight captain ignores the [[Conditions/Concealed|Concealed]] condition from smoke."

abilities_mid:
  - name: ""
  - name: "[[Bestiary Ability Glossary/Attack of Opportunity|Attack of Opportunity]]"
    desc: "`pf2:r`  "

  - name: "Final Spite"
    desc: "`pf2:r` (auditory,divine) **Trigger** The wight captain is reduced to 0 Hit Points\n**Effect** The wight captain blows a battered war horn before being destroyed. The unnerving blast summons reinforcements, raising 1d4 corpses in a 200-foot emanation as [[Bestiary 1/Wight|Wights]]."

  - name: "[[Bestiary Ability Glossary/Shield Block|Shield Block]]"
    desc: "`pf2:r`  "

attacks:
  - name: ""

  - name: "Melee"
    desc: "`pf2:1` Longsword +26 (magical, versatile p)\n__Damage__  2d8 + 13 slashing plus drain-life"

  - name: "Melee"
    desc: "`pf2:1` Claw +25 (agile)\n__Damage__  2d8 + 13 slashing plus drain-life"

  - name: "Divine Innate Spells"
    desc: "DC 28, attack +20\n__Constant__  __(5th)__ _[[Spells/Truespeech|Tongues]]_"

  - name: "Coordinated Strike"
    desc: "  The wight captain flanks with an ally as long as the target is within both their reaches, even if commander and ally aren't on opposite sides."

  - name: "Drain Life"
    desc: " (divine) When the wight captain damages a living creature using an unarmed attack or their bound weapon, they gain 11 temporary Hit Points, and the creature must succeed at a DC 28 Fortitude check save or become [[Conditions/Drained 1|Drained 1]]. Further damage dealt by the hunter wight's unarmed and bound weapon attacks increases the value of the drained condition by 1 on a failed save, to a maximum of drained 4."

  - name: "Spawn Wight Soldier"
    desc: " (divine) A living humanoid killed by a wight captain's weapon or claw Strike rises as a stalker wight after 1d4 rounds. This spawn is under the command of the wight commander that killed them. They don't have drain life or spawn stalker wight and are [[Conditions/Clumsy 1|Clumsy 2]] for as long as they're a wight spawn. If the creator of the stalker wight spawn dies, the wight spawn becomes a fully autonomous stalker wight; they regain their free will, gain drain life and spawn stalker wight, and are no longer clumsy."

  - name: "Tactical Direction"
    desc: "`pf2:2` (auditory) Each wight ally in a 30-foot emanation gains the [[Bestiary Ability Glossary/Attack of Opportunity|Attack of Opportunity]] reaction until the end of their next turn."
 
```

```encounter-table
name: Elite Smokesight Wight Captain
creatures:
  - 1: Elite Smokesight Wight Captain
```
### Description
Variant wight commander

### Description
When the beloved leader of a military unit is raised as a wight, sometimes the spirits of their comrades return with them, creating a gestalt being of impeccable tactical acumen.

* * *

Wights are undead beings of spite who primarily stalk places related to death.
