---
title: "Zridi"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.the-enmity-cycle-bestiary.Actor.xwEcqgkn9Fvrhpv7" 
tags:
  - pf2e/creature/type/div
  - pf2e/creature/type/evil
  - pf2e/creature/type/fiend
  - pf2e/creature/type/lawful
  - pf2e/creature/type/unholy
  - pf2e/creature/level/8
statblock: inline
name: "Zridi"
level: 8
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder Adventure: The Enmity Cycle"
name: "Zridi"
level: "Creature 8"
rare_03: "Unique"
alignment: ""
size: "Medium"
trait_01: "div"
trait_02: "evil"
trait_03: "fiend"
trait_04: "lawful"
trait_05: "unholy"
modifier: 16
perception:
  - name: "Perception"
    desc: "+16; Greater Darkvision"
languages: "Daemonic, Osiriani"
skills:
  - name: "Skills"
    desc: "Acrobatics: +15, Arcana: +14, Deception: +21, Diplomacy: +21, Intimidation: +17, Religion: +14, Society: +14, Stealth: +17"
abilityMods: [3, 5, 3, 3, 4, 7]
speed: 25 feet,  fly 35 feet
sourcebook: "_Pathfinder Adventure: The Enmity Cycle_"
ac: 26
armorclass:
  - name: AC
    desc: "26; __Fort__ +13, __Ref__ +17, __Will__ +18; +1 status to all saves vs. magic"
hp: 125
health:
  - name: ""
  - name: HP
    desc: "125; __Immunities__  disease; __Weaknesses__ cold iron 6, holy 6"
abilities_top:
  - name: ""
  - name: "Items"
    desc: "[[Equipment/Cloud Pouch|Horn of Fog]]"
abilities_mid:
  - name: ""
  - name: "Hatred of Red"
    desc: "  Pairakas hate the color red. Zridi won't wear the color or willingly enter any place painted red. Given a choice, she'll attack a creature wearing red first. If barred from expressing her displeasure toward the color by force or some magical effect, she takes 2d6 mental damage at the end of her turn."

attacks:
  - name: ""

  - name: "Melee"
    desc: "`pf2:1` Claw +18 (agile, finesse, magical, unholy)\n__Damage__  2d8 + 7 slashing plus bubonic-plague 1d6 spirit plus bubonic-plague"

  - name: "Divine Innate Spells"
    desc: "DC 26, attack +18; __4th __  _[[Spells/Charm|Charm (At Will)]]_, _[[Spells/Translocate|Dimension Door (At Will)]]_, _[[Compendium.pf2e.spells-srd.Item.c2bTWBNO1BYX4Zfg|Misdirection (At Will) (Self Only)]]_, _[[Spells/Outcast's Curse|Outcast's Curse (At Will)]]_, _[[Spells/Suggestion|Suggestion (At Will)]]_\n__Cantrips__  __(4th)__ _[[Spells/Detect Magic|Detect Magic]]_"

  - name: "Rituals"
    desc: "_[[Spells/Div Pact|Div Pact]]_"

  - name: "[[Compendium.pf2e.pathfinder-bestiary-3.XgCQnswAedPcwLck.Item.OnmLcXNYU1Jxp5ji|Bubonic Plague]]"
    desc: " (disease) A creature can't remove the fatigued condition while infected\n**Saving Throw** DC 24 Fortitude check\n**Onset** 1 day\n**Stage 1** [[Conditions/Fatigued|Fatigued]] (1 day)\n**Stage 2** [[Conditions/Enfeebled 1|Enfeebled 2]] and fatigued (1 day)\n**Stage 3** [[Conditions/Enfeebled 1|Enfeebled 3]], fatigued, and take 1d6 bleed every 1d20 minutes (1 day)"

  - name: "[[Bestiary Ability Glossary/Change Shape|Change Shape]]"
    desc: "`pf2:1` (concentrate,divine,polymorph) Zridi can take the appearance of any Small or Medium humanoid or animal. This doesn't change her Speed or her attack and damage modifiers with the Strikes, but it might change the damage type her Strikes deal.\n"

  - name: "Compelled Performance"
    desc: "`pf2:2` (divine,emotion,mental) **Frequency** once per day\n**Effect** Zridi grants the target a +2 status bonus to Performance checks for one week. The target must attempt a DC 26 Will check save.\n**Critical Success** No further effect.\n**Success** The target must [[Actions/Perform|Perform]] for 1 action on its next turn.\n**Failure** The target must Perform for 3 actions on its next turn, undertaking a Performance task set by Zridi.\n**Critical Failure** As failure, but Zridi can choose to delay the effect and trigger it at any point in the next week as a reaction.\n[[Bestiary Effects/Effect_ Compelled Performance|Effect: Compelled Performance]]"

  - name: "Tormenting Dreams"
    desc: "`pf2:2` (divine,emotion,mental) **Frequency** once per day\n**Effect** Zridi torments a sleeping creature within 100 feet with visions of betrayals by loved ones and friends. The target must attempt a DC 26 Will check save, with the effects of the [[Spells/Nightmare|Nightmare]] spell."
 
```

```encounter-table
name: Zridi
creatures:
  - 1: Zridi
```
### Description
Female pairaka


