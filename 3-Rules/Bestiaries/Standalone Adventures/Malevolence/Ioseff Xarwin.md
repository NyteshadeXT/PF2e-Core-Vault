---
title: "Ioseff Xarwin"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.malevolence-bestiary.Actor.n3taMFiViyEQ5JAi" 
tags:
  - pf2e/creature/type/chaotic
  - pf2e/creature/type/evil
  - pf2e/creature/type/ghost
  - pf2e/creature/type/incorporeal
  - pf2e/creature/type/spirit
  - pf2e/creature/type/undead
  - pf2e/creature/type/unholy
  - pf2e/creature/level/9
statblock: inline
name: "Ioseff Xarwin"
level: 9
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder Adventure: Malevolence"
name: "Ioseff Xarwin"
level: "Creature 9"
rare_03: "Unique"
alignment: ""
size: "Medium"
trait_01: "chaotic"
trait_02: "evil"
trait_03: "ghost"
trait_04: "incorporeal"
trait_05: "spirit"
trait_06: "undead"
trait_07: "unholy"
modifier: 16
perception:
  - name: "Perception"
    desc: "+16; Darkvision"
languages: "Aklo, Common, Diabolic"
skills:
  - name: "Skills"
    desc: "Arcana: +17, Astronomy Lore: +21, Cheliax Lore: +17, Deception: +19, Intimidation: +19, Occultism: +21, Society: +19, Stealth: +18"
abilityMods: [-5, 3, 0, 6, 3, 4]
speed:  fly 25 feet
sourcebook: "_Pathfinder Adventure: Malevolence_"
ac: 29
armorclass:
  - name: AC
    desc: "29; __Fort__ +15, __Ref__ +18, __Will__ +18"
hp: 112
health:
  - name: ""
  - name: HP
    desc: "112, void healing, rejuvination; __Immunities__  paralyzed,  unconscious,  poison,  precision,  death effects,  disease; __Resistances__ all damage 10 (except force, ghost touch, or vitality; double resistance vs. non-magical)"
abilities_top:
  - name: ""

  - name: "Site Bound"
    desc: "  Ioseff Xarwin is site bound to his laboratory."

abilities_mid:
  - name: ""
  - name: "Recoil"
    desc: "  If a character forcefully presents Fulvia's ring to Ioseff as an action with the manipulate trait, he must attempt a DC 26 Will check save; he's thereafter immune to Recoil for 24 hours.\n**Critical Success** Ioseff is unaffected.\n**Success** Ioseff takes 4d6 mental damage and is [[Conditions/Frightened 1|Frightened 1]].\n**Failure** Ioseff takes 8d6 mental damage and is [[Conditions/Frightened 1|Frightened 2]].\n**Critical Failure** Ioseff takes 8d6 mental damage, is frightened 2, and is [[Conditions/Stunned 1|Stunned 3]]."

  - name: "Rejuvenation"
    desc: " (divine) When a ghost is destroyed, it re-forms after 2d4 days within the location it's bound to, fully healed. A ghost can be permanently destroyed only if someone determines the reason for its existence and sets right whatever prevents the spirit from resting.\nIn order to put Ioseff to rest, the following three conditions must be met after defeating Ioseff but before he rejuvenates:\n*   The undead brain collector in area D6 has been destroyed.\n*   The brains of his wife and children have been freed from the brain collector statue, allowing their souls to move to the Boneyard.\n*   The Void Mirror has been replaced in its frame in the Cosmic Crypt."

  - name: "Shame"
    desc: "  Ioseff takes a -1 item penalty to attack rolls and saving throws against a character who carries Fulvia's ring. This penalty doubles to -2 against a character who wears Fulvia's ring. At the start of ttheir turn, a character who possesses Fulvia's ring can attempt a secret DC 20 Perception check check; on a success, they sense a faint compulsion fueled by outrage to present Fulvia's ring to the ghost."

attacks:
  - name: ""

  - name: "Melee"
    desc: "`pf2:1` Bloody Ectoplasmic Lash +19 (agile, finesse, magical, reach 10 feet)\n__Damage__  2d8 + 7 void"

  - name: "Innate Occult Spells"
    desc: "DC 28, attack +20; __5th __ (2 slots) _[[Spells/Banishment|Banishment]]_, _[[Spells/Summon Entity|Summon Entity]]_; __4th __ (3 slots) _[[Spells/Grim Tendrils|Grim Tendrils]]_, _[[Spells/Nightmare|Nightmare]]_, _[[Spells/Phantasmal Killer|Phantasmal Killer]]_; __3rd __ (3 slots) _[[Spells/Dream Message|Dream Message]]_, _[[Spells/Paralyze|Paralyze]]_, _[[Spells/Slow|Slow]]_; __2nd __ (3 slots) _[[Spells/Peaceful Rest|Gentle Repose]]_, _[[Spells/Mirror Image|Mirror Image]]_, _[[Spells/Telekinetic Maneuver|Telekinetic Maneuver]]_; __1st __ (3 slots) _[[Spells/Command|Command]]_, _[[Spells/Fear|Fear]]_, _[[Spells/Enfeeble|Ray of Enfeeblement]]_\n__Cantrips__  __(5th)__ _[[Spells/Daze|Daze]]_, _[[Spells/Ghost Sound|Ghost Sound]]_, _[[Spells/Light|Light]]_, _[[Spells/Telekinetic Hand|Mage Hand]]_, _[[Spells/Telekinetic Projectile|Telekinetic Projectile]]_"

  - name: "Rituals"
    desc: "_[[Spells/Extract Brain|Extract Brain]]_, _[[Spells/Inveigle|Inveigle]]_"

  - name: "Incite Insurrection"
    desc: "`pf2:r`  **Trigger** A creature within 30 feet of Ioseff fails a saving throw against one of his spells\n**Effect** Ioseff infuses the magic of his spell with the agonies of his own death. In addition to any effects caused by the triggering spell, the target must also attempt a DC 28 Fortitude check save. or contract internal insurrection at stage 1 (stage 2 on a critical failure)\nInternal Insurrection (disease); Level 7.\nThe target can't recover from the clumsy condition from internal insurrection until the disease is cured\n**Stage 1** [[Conditions/Clumsy 1|Clumsy 1]] (1 day)\n**Stage 2** [[Conditions/Clumsy 1|Clumsy 2]] (1 day)\n**Stage 3** clumsy 2 and [[Conditions/Off-Guard|Off-Guard]] (1 day)\n**Stage 4** [[Conditions/Clumsy 1|Clumsy 3]] and off-guard (1 day)\n**Stage 5** death, and the target's body splits apart into a mass of fragments that crawls outward in a 10-foot-emanation before perishing, leaving a bare skeleton surrounded by gore."

  - name: "Malevolent Possession"
    desc: "`pf2:2`  Ioseff Xarwin attempts to possess an adjacent corporeal creature. This has the same effect as the [[Spells/Possession|Possession]] spell, except since Ioseff Xarwin doesn't have a physical body, it is unaffected by that restriction of the spell.\nThe target must attempt a DC 28 Will check save.\n**Critical Success** The target is unaffected.\n**Success** You possess the target but can't control it. You ride along in the body while the spell lasts.\n**Failure** You possess the target and take partial control of it. You no longer have a separate turn; instead, you might control the target. At the start of each of the target's turns, it attempts another Will save. If it fails, it's [[Conditions/Controlled|Controlled]] by you on that turn; if it succeeds, it chooses its own actions; and if it critically succeeds, it forces you out and the spell ends.\n**Critical Failure** You possess the target fully, and it can only watch as you manipulate it like a puppet. The target is Controlled by you.\nA character who carries Fulvia's ring on their person gains a +4 item bonus to their save and treats the result of the save as one degree of success better than their rolled result. If Ioseff attempts to use this ability on a character who wears Fulvia's ring, he can attempt a Perception check against the ring-wearer's Stealth DC to notice the ring in time to avoid the attempt; on a success, he loses the two actions he took to make the attempt but avoids further effects. If he doesn't notice the ring and attempts to possess the ring-wearer, the ring-wearer automatically gains a critical success on their Will save, Ioseff is [[Conditions/Stunned 1|Stunned 3]], and he can't use Malevolent Possession again for 24 hours."

  - name: "Phantasmal Amputation"
    desc: "`pf2:1` (occult) **Frequency** once per day\n**Effect** Ioseff holds up the stump of his left arm, and the streaming blood begins to form a ghostly hand that he then uses to point out a humanoid creature within 30 feet. That creature must attempt a DC 28 Fortitude check save.\n**Critical Success** The creature is [[Conditions/Sickened 1|Sickened 1]] by a sudden, agonizing pain in their left hand.\n**Success** As critical success, but the creature also takes 2d6 slashing damage and is [[Conditions/Clumsy 1|Clumsy 1]] for 1 round as they feel as if their left hand just got lopped off by a hatchet.\n**Failure** The creature is [[Conditions/Sickened 1|Sickened 2]], takes 5d6 slashing damage, and is [[Conditions/Clumsy 1|Clumsy 2]] for 1 minute as they feel as if their left hand just got lopped off by a hatchet.\n**Critical Failure** The PC is [[Conditions/Sickened 1|Sickened 3]] and takes 8d6 slashing damage and 2d6 bleed damage as their left hand appears to detach and fall to the floor before instantly decaying into bones. The PC is clumsy 2 and [[Conditions/Wounded 1|Wounded 1]]. As soon as this wounded condition is removed, the PC's hand miraculously returns and the clumsy condition ends."
 
```




