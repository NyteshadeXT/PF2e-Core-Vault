---
title: "Vincuvicar"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.stolen-fate-bestiary.Actor.fcMf5NrHPZS54Hkd" 
tags:
  - pf2e/creature/type/evil
  - pf2e/creature/type/fiend
  - pf2e/creature/type/lawful
  - pf2e/creature/type/unholy
  - pf2e/creature/type/velstrac
  - pf2e/creature/level/18
statblock: inline
name: "Vincuvicar"
level: 18
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder #191: The Destiny War"
name: "Vincuvicar"
level: "Creature 18"
rare_03: "Uncommon"
alignment: ""
size: "Medium"
trait_01: "evil"
trait_02: "fiend"
trait_03: "lawful"
trait_04: "unholy"
trait_05: "velstrac"
modifier: 32
perception:
  - name: "Perception"
    desc: "+32; Greater Darkvision, Truesight"
languages: "Common, Diabolic, Shadowtongue"
skills:
  - name: "Skills"
    desc: "Acrobatics: +31, Athletics: +35, Crafting: +29, Deception: +32, Intimidation: +34, Medicine: +32, Religion: +30, Stealth: +31, Survival: +30, Torture Lore: +31"
abilityMods: [9, 6, 6, 5, 6, 6]
speed: 25 feet
sourcebook: "_Pathfinder #191: The Destiny War_"
ac: 42
armorclass:
  - name: AC
    desc: "42; __Fort__ +32, __Ref__ +28, __Will__ +30; +1 status to all saves vs. magic"
hp: 333
health:
  - name: ""
  - name: HP
    desc: "333, regeneration 25 (deactivated by holy or silver); __Immunities__  cold,  fear effects; __Weaknesses__ holy 20, silver 20"
abilities_top:
  - name: ""
  - name: "Items"
    desc: "[[Equipment/War Flail|+2 Greater Striking War Flail]], [[Equipment/Full Plate|+2 Greater Resilient Full Plate]], 6x [[Equipment/Manacles (Average)|Manacles (Average)]]"
  - name: "Painsight"
    desc: "  A velstrac automatically knows whether a creature it sees has any of the [[Conditions/Doomed 1|Doomed]], [[Conditions/Dying 1|Dying]], and [[Conditions/Wounded 1|Wounded]] conditions, as well as the value of those conditions."

abilities_mid:
  - name: ""
  - name: "Alone and Forgotten"
    desc: " (aura,divine,fear,mental) 30 feet. When a creature ends its turn in the aura, it feels imprisoned and must succeed at a DC 37 Will check save or become [[Conditions/Enfeebled 1|Enfeebled 1]] and [[Conditions/Slowed 1|Slowed 1]] for 1 round."

  - name: "[[Bestiary Ability Glossary/Attack of Opportunity|Attack of Opportunity]]"
    desc: "`pf2:r`  "

attacks:
  - name: ""

  - name: "Melee"
    desc: "`pf2:1` War Flail +35 (disarm, magical, sweep, trip, unholy)\n__Damage__  2d6 bleed 3d10 + 17 bludgeoning"

  - name: "Ranged"
    desc: "`pf2:1` Animated Manacles +32 (magical, range increment 20 feet, unholy)\n__Damage__  3d8 + 8 piercing 2d6 bleed"

  - name: "Divine Innate Spells"
    desc: "DC 40, attack +32; __9th __  _[[Spells/Darkness|Darkness (At Will)]]_, _[[Spells/Divine Wrath|Divine Wrath]]_, _[[Spells/Scouting Eye|Prying Eye]]_; __8th __  _[[Spells/Command|Command]]_, _[[Spells/Hidden Mind|Mind Blank]]_, _[[Spells/Mind Probe|Mind Probe]]_, _[[Spells/Teleport|Teleport]]_\n__Cantrips__  __(9th)__ _[[Spells/Daze|Daze]]_, _[[Spells/Sigil|Sigil]]_\n__Constant__  __(9th)__ _[[Spells/Discern Lies|Discern Lies]]_, _[[Spells/Truesight|True Seeing]]_"

  - name: "Rituals"
    desc: "_[[Spells/Imprisonment|Imprisonment]]_"

  - name: "Focus Gaze"
    desc: "`pf2:1` (concentrate,divine,fear,mental,visual) The vincuvicar stares at a creature they can see within 30 feet. The creature must attempt a Will save against the alone and forgotten aura. If the creature was already enfeebled and slowed, on a failed save, its hopelessness causes it to be [[Conditions/Enfeebled 1|Enfeebled 2]] and unable to take the [[Actions/Escape|Escape]] action as long as they remain enfeebled. After attempting this save, the creature is then temporarily immune until the start of the vincuvicar's next turn."

  - name: "Institutionalized"
    desc: "  A vincuvicar forms a bond with an institution that holds prisoners. This takes 1 day and ends any previous bond the vincuvicar had. While within the area of the prison, the vincuvicar gains a +2 status bonus to skill checks and Perception checks."

  - name: "Shackle"
    desc: "`pf2:r`  **Trigger** A creature within 60 feet is damaged by the vincuvicar's animated manacles Strike\n**Effect** The creature must attempt a DC 40 Reflex check save.\n**Critical Success** The creature avoids being manacled.\n**Success** The manacles distract the creature, causing it to become [[Conditions/Off-Guard|Off-Guard]] until the end of its next turn.\n**Failure** The manacles fuse in place and bind the creature's legs and arms until it [[Actions/Escape|Escapes]] (DC 40) or the manacles are destroyed.\n**Critical Failure** As failure, but the creature is also [[Conditions/Restrained|Restrained]]."
 
```



Prison keepers among velstracs, vincuvicar velstracs thrive on depriving others of freedom.

## Vincuvicar Thrones

Each vincuvicar possesses a personal, spiked iron throne—a symbol of authority. This iron throne is an object with Hardness 20 and 500 Hit Points. If a vincuvicar's throne is destroyed, the velstrac must return to the Shadow Plane to acquire or construct a replacement.
