---
title: Tusker Riders
obsidianUIMode: preview
noteType: pf2eArmy
cssClasses: pf2e
tags:
  - pf2eArmy

source: 
aliases: "Compendium.pf2e.kingmaker-bestiary.Actor.s8Ol97yji6CbWt1l" 
level: 14
license: 
statblock: inline
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: ""
name: "Tusker Riders"
level: "Army 14"


alignment: ""
size: ""
trait_01: infantry
modifier: 24
perception:
  - name: ""
  - name: "Scouting"
    desc: "+24"
  - name: "Recruitment DC"
    desc: "15; __Consumption__ 1"
  - name: "Description"
    desc: "The hill giant warlord Kob Moleg sent these mammoth-mounted hill giants to King Irovetti as a gift."
ac: 35
armorclass:
  - name: AC
    desc: "35; __Maneuver__ +22, __Morale__ +28"
hp: 8
health:
  - name: ""
  - name: "HP"
    desc: "8 (RT 4))"
abilities_top:
  - name: ""

abilities_mid:
  - name: ""
  - name: "Melee"
    desc: "Morningstars and tusks +18"
  - name: "Ranged"
    desc: "Thrown rock +18"

attacks:
  - name: ""

  - name: "Tusker Training"
    desc: "The Tusker Riders can use the [[Kingmaker Features/All-Out Assault|All-Out Assault]], [[Kingmaker Features/Covering Fire|Covering Fire]], and [[Kingmaker Features/Taunt|Taunt]] tactical actions."

  - name: "Trampling Charge"
    desc: ""

  - name: "Increased Ammunition"
    desc: "You increase the number of times your army can use ranged Strikes in each war encounter by 2. This tactic can be taken multiple times; each time you do so, increase the army's maximum number of ranged Strikes by 2."

  - name: "Darkvision"
    desc: "The army includes several spotters and scouts who have darkvision, and the rest of the soldiers have been trained to follow their lead so that the army itself functions as if it had darkvision."

  - name: "All-Out Assault"
    desc: "`pf2:2` **Requirement** [[Kingmaker Features/Merciless|Merciless]]\n* * *\n\nYour army attacks with frightening vigor. Attempt a melee Strike against an enemy army's AC.\n\n**Critical Success** Your army inflicts 3 points of damage to the target army. If your army's next war action this turn is an attack war action against a different target army, you gain a +1 circumstance bonus to the Strike as your fury continues to the new target.\n\n**Success** Your army deals 2 points of damage to the target army.\n\n**Failure** Your army falters, but still deals 1 point of damage to the target army.\n\n**Critical Failure** Your army deals no damage to the target army and becomes [[Kingmaker Features/Outflanked|Outflanked]] until the start of its next turn."

  - name: "Covering Fire"
    desc: "`pf2:2` **Requirement** [[Kingmaker Features/Sharpshooter|Sharpshooter]]\n* * *\n\nYour army's ranged fire provides cover and protection for an allied army to maneuver. Attempt a ranged Strike against a target army's AC.\n\n**Critical Success** You inflict 2 points of damage to the target army, and it cannot take reactions triggered by maneuver war actions from any army until the start of your next turn.\n\n**Success** You inflict 1 point of damage to the target army, and it can't take reactions triggered by maneuver war actions from any army until the start of your next turn.\n\n**Failure** Your attack fails to provide covering fire, but you inflict 1 point of damage to the target army.\n\n**Critical Failure** Your attempt fails."

  - name: "Taunt"
    desc: "`pf2:1` **Requirement**[[Kingmaker Features/Focused Devotion|Focused Devotion]]\n* * *\n\nYour army attempts to frighten and cow an enemy army. Attempt a Morale check check against the target army.\n\n**Critical Success** The target army becomes [[Kingmaker Features/Shaken|Shaken 2]] until the start of your next turn.\n\n**Success** The target army becomes shaken 1 until the start of your next turn.\n\n**Critical Failure** Your failed attempt bolsters the enemy's spirits. This reduces the target army's shaken value by 1."

  - name: "Trampling Charge"
    desc: "`pf2:3` The Riders trample an enemy army. They attempt a Maneuver check against a target non-engaged army's Maneuver DC. Trampling Charge does not trigger [[Kingmaker Features/Counterattack|Counterattack]] reactions.\n\n**Critical Success** The target army takes 2 points of damage and increases their [[Kingmaker Features/Shaken|Shaken]] value by 1.\n\n**Success** The target army takes 1 point of damage.\n\n**Failure** The target army takes 1 point of damage. The Tusker Riders are now [[Kingmaker Features/Engaged|Engaged]] with the target army.\n\n**Critical Failure** The Tusker Riders are now [[Kingmaker Features/Engaged|Engaged]] with the target army and are off-guard until the start of their next turn."
 
```

```encounter-table
name: Tusker Riders
creatures:
  - 1: Tusker Riders
```

