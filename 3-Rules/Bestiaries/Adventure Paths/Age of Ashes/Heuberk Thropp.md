---
title: "Heuberk Thropp"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.age-of-ashes-bestiary.Actor.dlW3UaXVpnzjd6xe" 
tags:
  - pf2e/creature/type/evil
  - pf2e/creature/type/human
  - pf2e/creature/type/humanoid
  - pf2e/creature/type/lawful
  - pf2eMonster
  - pf2e/creature/level/9
statblock: inline
name: "Heuberk Thropp"
level: 9
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder #147: Tomorrow Must Burn"
name: "Heuberk Thropp"
level: "Creature 9"
rare_03: [[Unique]]
alignment: ""
size: "Medium"
trait_01: [[evil]]
trait_02: [[human]]
trait_03: [[humanoid]]
trait_04: [[lawful]]
modifier: 14
perception:
  - name: "Perception"
    desc: "+14; "
languages: "Common, Dwarven, Elven, Halfling, Kholo"
skills:
  - name: "Skills"
    desc: "Athletics: +16, Deception: +21, Diplomacy: +15, Intimidation: +19, Society: +16, Thievery: +16, Mercantile Lore: +16"
abilityMods: [3, 2, 3, 1, 1, 4]
speed: 25 feet
sourcebook: "_Pathfinder #147: Tomorrow Must Burn_"
ac: 28
armorclass:
  - name: AC
    desc: "28; __Fort__ +21, __Ref__ +15, __Will__ +18"
hp: 155
health:
  - name: ""
  - name: HP
    desc: "155"
abilities_top:
  - name: ""
  - name: "Items"
    desc: "3x [[Equipment/Dagger|Dagger]], [[Equipment/Mace|+1 Striking Mace]], [[Equipment/Leather Armor|Leather Armor]], Key to Manacles, [[Equipment/Manacles (Average)|Manacles (Average) (Marked with the Symbol of the Scarlet Triad)]], [[Equipment/Spellbook (Blank)|Spellbook]], [[Equipment/Thieves' Toolkit (Infiltrator)|Thieves' Tools (Infiltrator)]]"
abilities_mid:
  - name: ""
  - name: "Timely Distraction"
    desc: "`pf2:r`  **Trigger** A creature Heuberk can see within his melee reach misses him with a melee Strike.\n* * *\n\n**Effect** Heuberk attempts to [[Actions/Feint|Feint]] the creature. If it's not Heuberk's turn and he gets a success, the effect applies on his next turn."

attacks:
  - name: ""

  - name: "**Melee** `pf2:1` Mace"
    desc: "+19 (shove)\n__Damage__  2d6 + 9 bludgeoning"

  - name: "**Melee** `pf2:1` Dagger"
    desc: "+18 (agile, versatile s)\n__Damage__  1d4 + 9 piercing"

  - name: "**Ranged** `pf2:1` Dagger"
    desc: "+18 (agile, thrown 10 ft., versatile s)\n__Damage__  1d4 + 9 piercing"

  - name: "Occult Prepared Spells"
    desc: "DC 27, attack +19; __5th __  _[[Spells/Wave of Despair|Crushing Despair]]_; __4th __  _[[Spells/Discern Lies|Discern Lies]]_, _[[Spells/Fly|Fly]]_; __3rd __  _[[Spells/Dream Message|Dream Message]]_, _[[Spells/Enthrall|Enthrall]]_, _[[Spells/Hypnotize|Hypnotic Pattern]]_; __2nd __  _[[Spells/Invisibility|Invisibility]]_, _[[Spells/Mirror Image|Mirror Image]]_, _[[Spells/Paranoia|Paranoia]]_; __1st __  _[[Spells/Charm|Charm]]_, _[[Spells/Command|Command]]_, _[[Spells/Fear|Fear]]_, _[[Spells/Enfeeble|Ray of Enfeeblement]]_\n__Cantrips__  __(5th)__ _[[Spells/Detect Magic|Detect Magic]]_, _[[Spells/Message|Message]]_, _[[Spells/Prestidigitation|Prestidigitation]]_, _[[Spells/Read Aura|Read Aura]]_, _[[Spells/Shield|Shield]]_"

  - name: "Identify an Opening"
    desc: "`pf2:1` (auditory,linguistic) **Requirements** A creature is [[Conditions/Off-Guard|Off-Guard]] against Heuberk's next melee attack.\n* * *\n\n**Effect** Heuberk points out a lapse in the creature's defenses. The creature is off-guard against all melee attacks attempted by any of Heuberk's allies who hear and understand this instruction until the start of Heuberk's next turn."

  - name: "[[Bestiary Ability Glossary/Sneak Attack|Sneak Attack]]"
    desc: "  Hueberk deals an extra 2d6 precision damage to [[Conditions/Off-Guard|Off-Guard]] creatures."
 
```

```encounter-table
name: Heuberk Thropp
creatures:
  - 1: Heuberk Thropp
```




