---
title: "Dmiri Yoltosha"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.age-of-ashes-bestiary.Actor.L4K4V09tQVXj7ZiI" 
tags:
  - pf2e/creature/type/evil
  - pf2e/creature/type/goblin
  - pf2e/creature/type/humanoid
  - pf2e/creature/type/lawful
  - pf2eMonster
  - pf2e/creature/level/4
statblock: inline
name: "Dmiri Yoltosha"
level: 4
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder #145: Hellknight Hill"
name: "Dmiri Yoltosha"
level: "Creature 4"
rare_03: [[Unique]]
alignment: ""
size: "Medium"
trait_01: [[evil]]
trait_02: [[goblin]]
trait_03: [[humanoid]]
trait_04: [[lawful]]
modifier: 14
perception:
  - name: "Perception"
    desc: "+14; Darkvision"
languages: "Common, Goblin"
skills:
  - name: "Skills"
    desc: "Acrobatics: +10, Athletics: +9, Deception: +5, Nature: +7, Stealth: +10, Survival: +7, Thievery: +10"
abilityMods: [3, 4, 1, 0, 1, -1]
speed: 25 feet
sourcebook: "_Pathfinder #145: Hellknight Hill_"
ac: 22
armorclass:
  - name: AC
    desc: "22; __Fort__ +10, __Ref__ +12, __Will__ +9"
hp: 52
health:
  - name: ""
  - name: HP
    desc: "52"
abilities_top:
  - name: ""
  - name: "Items"
    desc: "[[Equipment/Composite Shortbow|Composite Shortbow]], [[Equipment/Dagger|Dagger]], [[Equipment/Rapier|+1 Striking Rapier]], [[Equipment/Leather Armor|+1 Leather Armor]], Key to Chest, 20x [[Equipment/Arrows|Arrows]]"
abilities_mid:
  - name: ""
attacks:
  - name: ""

  - name: "**Melee** `pf2:1` Rapier"
    desc: "+13 (deadly d8, disarm, finesse)\n__Damage__  2d6 + 3 piercing"

  - name: "**Melee** `pf2:1` Dagger"
    desc: "+12 (agile, finesse, versatile s)\n__Damage__  1d4 + 3 piercing"

  - name: "**Ranged** `pf2:1` Dagger"
    desc: "+12 (agile, thrown 10 ft., versatile s)\n__Damage__  1d4 + 3 piercing"

  - name: "**Ranged** `pf2:1` Composite Shortbow"
    desc: "+12 (deadly d10, range increment 60 feet, reload 0)\n__Damage__  1d6 + 1 piercing"

  - name: "Flurry"
    desc: "  Dmiri's multiple attack penalty against her hunted prey is -3 (-2 with an agile weapon) on her second attack and -6 (-4 with an agile weapon) on her third or subsequent attacks."

  - name: "Hunt Prey"
    desc: "`pf2:1`  Dmiri designates a single creature as her prey and focuses her attacks against that creature. Dmiri must be able to see or hear the prey, or she must be tracking the prey during exploration.\n\nDmiri gains a +2 circumstance bonus to Perception checks when she [[Actions/Seek|Seeks]] her prey and a +2 circumstance bonus to Survival checks when she [[Actions/Track|Tracks]] her prey. Dmiri also ignores the penalty for making ranged attacks within her second range increment against the prey she is hunting.\n\nDmiri can have only one creature designated as prey at a time. If she uses Hunt Prey against a creature when she already has a creature designated, the prior creature loses the designation and the new prey gains the designation. Dmiri's designation lasts until her next daily preparations."

  - name: "Hunter's Aim"
    desc: "`pf2:2` (concentrate,ranger) When Dmiri focuses on aiming, her attack becomes particularly accurate. Make a ranged weapon Strike against her hunted prey. On this Strike, Dmiri gains a +2 circumstance bonus to the attack roll and ignores her prey's [[Conditions/Concealed|Concealed]] condition."

  - name: "Twin Parry"
    desc: "`pf2:1`  **Requirements** Dmiri is wielding two melee weapons, one in each hand\n* * *\n\nDmiri uses her two weapons to parry attacks. Dmiri gains a +1 circumstance bonus to AC until the start of her next turn, or a +2 circumstance bonus if either weapon has the parry trait. Dmiri loses this circumstance bonus if she no longer meet this ability's requirement.\n\n[[Feat_Feature Effects/Effect_ Twin Parry|Effect: Twin Parry]]\n\n[[Feat_Feature Effects/Effect_ Twin Parry (Parry Trait)|Effect: Twin Parry (Parry Trait)]]"

  - name: "Twin Takedown"
    desc: "`pf2:1`  **Frequency** once per round\n\n**Requirement** Dmiri is wielding two melee weapons, each in a different hand\n* * *\n\nDmiri swiftly attacks her hunted prey with both weapons. Make two Strikes against Dmiri 's hunted prey, one with each of the required weapons. If both hit the same hunted prey, combine their damage for the purpose of its resistances and weaknesses. Apply Dmiri 's multiple attack penalty to each Strike normally."
 
```

```encounter-table
name: Dmiri Yoltosha
creatures:
  - 1: Dmiri Yoltosha
```


Female hobgoblin ranger 4


