---
title: "Keff The Lion (1-2)"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.pfs-season-1-bestiary.Actor.yn7x4Me1YrPuXFZf" 
tags:
  - pf2e/creature/type/animal
  - pf2e/creature/type/evil
  - pf2e/creature/level/3
statblock: inline
name: "Keff The Lion (1-2)"
level: 3
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder Society Scenario #1-14: Lions of Katapesh"
name: "Keff The Lion (1-2)"
level: "Creature 3"

alignment: ""
size: "Large"
trait_01: "animal"
trait_02: "evil"
modifier: 9
perception:
  - name: "Perception"
    desc: "+9; Low-Light Vision, Scent (Imprecise) 30 Feet"
skills:
  - name: "Skills"
    desc: "Acrobatics: +8, Arcana: +7, Athletics: +11, Diplomacy: +7, Stealth: +10"
abilityMods: [2, 2, 2, 2, 2, 2]
speed: 30 feet
sourcebook: "_Pathfinder Society Scenario #1-14: Lions of Katapesh_"
ac: 17
armorclass:
  - name: AC
    desc: "17; __Fort__ +9, __Ref__ +9, __Will__ +7"
hp: 40
health:
  - name: ""
  - name: HP
    desc: "40"
abilities_top:
  - name: ""

abilities_mid:
  - name: ""
attacks:
  - name: ""

  - name: "Melee"
    desc: "`pf2:1` Jaws +9 ()\n__Damage__  1d10 + 3 piercing plus grab"

  - name: "Melee"
    desc: "`pf2:1` Claws +9 (agile)\n__Damage__  1d8 + 3 slashing"

  - name: "Arcane Prepared Spells"
    desc: "DC 17, attack +9; __1st __  _[[Spells/Dizzying Colors|Color Spray]]_, _[[Spells/Grease|Grease]]_, _[[Spells/Sure Strike|True Strike]]_\n__Cantrips__  __(1st)__ _[[Spells/Detect Magic|Detect Magic]]_, _[[Spells/Electric Arc|Electric Arc]]_, _[[Spells/Telekinetic Hand|Mage Hand]]_, _[[Spells/Read Aura|Read Aura]]_, _[[Spells/Shield|Shield]]_"

  - name: "Pack Attack"
    desc: "  The lion deals 1d4 extra damage to creatures within reach of at least two of the lion's allies."

  - name: "Pounce"
    desc: "`pf2:1`  The lion Strides and makes a Strike at the end of that movement. If the lion began this action [[Conditions/Hidden|Hidden]], it remains hidden until after the ability's Strike."

  - name: "[[Bestiary Ability Glossary/Sneak Attack|Sneak Attack]]"
    desc: "  The lion deals 1d6 extra precision damage to [[Conditions/Off-Guard|Off-Guard]] creatures."

  - name: "[[Bestiary Ability Glossary/Grab|Grab]]"
    desc: "`pf2:1`  "
 
```




