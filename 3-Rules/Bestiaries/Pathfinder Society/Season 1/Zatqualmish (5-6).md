---
title: "Zatqualmish (5-6)"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.pfs-season-1-bestiary.Actor.UQyvGqfFcYva3fzc" 
tags:
  - pf2e/creature/type/evil
  - pf2e/creature/type/gnome
  - pf2e/creature/type/humanoid
  - pf2e/creature/level/6
statblock: inline
name: "Zatqualmish (5-6)"
level: 6
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder Society Quest #8: Shadows of the Black Sovereign"
name: "Zatqualmish (5-6)"
level: "Creature 6"
rare_03: "Unique"
alignment: ""
size: "Small"
trait_01: "evil"
trait_02: "gnome"
trait_03: "humanoid"
modifier: 12
perception:
  - name: "Perception"
    desc: "+12; Low-Light Vision"
languages: "Common, Dwarven, Elven, Fey, Gnomish"
skills:
  - name: "Skills"
    desc: "Crafting: +9, Deception: +14, Diplomacy: +12, Legal Lore: +9, Occultism: +11, Performance: +14, Society: +11 (+12 to Create Forgeries), Thievery: +11, Underworld Lore: +9"
abilityMods: [1, 3, 2, 1, 1, 4]
speed: 25 feet
sourcebook: "_Pathfinder Society Quest #8: Shadows of the Black Sovereign_"
ac: 24
armorclass:
  - name: AC
    desc: "24; __Fort__ +10, __Ref__ +15, __Will__ +13; +1 Status to All Saves vs. Illusions"
hp: 76
health:
  - name: ""
  - name: HP
    desc: "76"
abilities_top:
  - name: ""
  - name: "Items"
    desc: "[[Equipment/Shortbow|Shortbow]], [[Equipment/Rapier|+1 Rapier]], Wax and Official Seals (Pathfinder Society), [[Equipment/Clothing (Fine)|Clothing (Fine)]], [[Equipment/Writing Set|Writing Set]], [[Equipment/Writing Set (Extra Ink and Paper)|Writing Set (Extra Ink and Paper)]], [[Equipment/Musical Instrument (Handheld)|Piccolo]], [[Equipment/Thieves' Toolkit|Thieves' Tools]], [[Equipment/Healing Potion (Minor)|Healing Potion (Minor)]], 10x [[Equipment/Arrows|Arrows]]"
abilities_mid:
  - name: ""
  - name: "Run Away!"
    desc: "`pf2:r`  **Trigger** A creature ends its movement adjacent to Zatqualmish\n**Effect** Zatqualmish Strides up to 10 feet (or his Speed, if that Speed is less than 10 feet). He must end his movement in a location that isn't within 5 feet of a foe.\nThis movement doesn't trigger reactions."

attacks:
  - name: ""

  - name: "Melee"
    desc: "`pf2:1` Rapier +12 (deadly d8, disarm, finesse)\n__Damage__  1d6 + 1 piercing"

  - name: "Melee"
    desc: "`pf2:1` Shortbow +12 (deadly d10)\n__Damage__  1d6 + 1 piercing"

  - name: "Occult Spontaneous Spells"
    desc: "DC 25, attack +16; __3rd __ (3 slots) _[[Spells/Haste|Haste]]_, _[[Spells/Slow|Slow]]_, _[[Spells/Vampiric Feast|Vampiric Touch]]_; __2nd __ (3 slots) _[[Spells/Blur|Blur]]_, _[[Spells/Laughing Fit|Hideous Laughter]]_, _[[Spells/Invisibility|Invisibility]]_; __1st __ (3 slots) _[[Spells/Charm|Charm]]_, _[[Spells/Dizzying Colors|Color Spray]]_, _[[Spells/Force Barrage|Magic Missile]]_, _[[Spells/Phantasmal Minion|Unseen Servant]]_\n__Cantrips__  __(3rd)__ _[[Spells/Void Warp|Chill Touch]]_, _[[Spells/Detect Magic|Detect Magic]]_, _[[Spells/Guidance|Guidance]]_, _[[Spells/Read Aura|Read Aura]]_, _[[Spells/Shield|Shield]]_"

  - name: "Bard Composition Spells"
    desc: "2 Focus Points, DC 25, attack +15; __3rd __  _[[Spells/Counter Performance|Counter Performance]]_\n__Cantrips__  __(3rd)__ _[[Spells/Courageous Anthem|Inspire Courage]]_, _[[Spells/Triple Time|Triple Time]]_"
 
```




