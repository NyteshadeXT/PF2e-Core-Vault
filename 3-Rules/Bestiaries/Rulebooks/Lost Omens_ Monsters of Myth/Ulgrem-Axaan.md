---
title: "Ulgrem-Axaan"
obsidianUIMode: preview
noteType: pf2eMonster
cssClasses: pf2e
aliases: "Compendium.pf2e.lost-omens-monsters-of-myth-bestiary.Actor.jInkcz2UYdaCzsBU" 
tags:
  - pf2e/creature/type/amphibious
  - pf2e/creature/type/beast
  - pf2e/creature/type/chaotic
  - pf2e/creature/type/evil
  - pf2e/creature/type/undead
  - pf2e/creature/type/unholy
  - pf2e/creature/level/7
statblock: inline
name: "Ulgrem-Axaan"
level: 7
license: OGL
---

```statblock
columns: 2
forcecolumns: true
layout: Basic Pathfinder 2e Layout
source: "Pathfinder Lost Omens: Monsters of Myth"
name: "Ulgrem-Axaan"
level: "Creature 7"
rare_03: "Unique"
alignment: ""
size: "Large"
trait_01: "amphibious"
trait_02: "beast"
trait_03: "chaotic"
trait_04: "evil"
trait_05: "undead"
trait_06: "unholy"
modifier: 18
perception:
  - name: "Perception"
    desc: "+18; Darkvision, Wavesense (Imprecise) 60 Feet"
languages: "Necril, Wildsong"
skills:
  - name: "Skills"
    desc: "Athletics: +17, Deception: +15, Intimidation: +17, Stealth: +16 (+20 in water)"
abilityMods: [6, 4, 4, -2, -1, 4]
speed: 25 feet,  burrow 10 feet,  swim 30 feet
sourcebook: "_Pathfinder Lost Omens: Monsters of Myth_"
ac: 24
armorclass:
  - name: AC
    desc: "24; __Fort__ +18, __Ref__ +15, __Will__ +12; +2 status to all non-primal emotion effects"
hp: 130
health:
  - name: ""
  - name: HP
    desc: "130, void healing; __Immunities__  death effects,  disease,  paralyzed,  poison,  unconscious; __Weaknesses__ fire 5; __Resistances__ cold 10, electricity 5, piercing 5"
abilities_top:
  - name: ""

  - name: "Frozen Contemplation"
    desc: "  Being frozen stops the constant pain Ulgrem-Axaan has experienced since its death. Ulgrem-Axaan has fast healing 5 while in severe cold or colder environments, or for 1 round after taking cold damage."

abilities_mid:
  - name: ""
  - name: "Baleful Glare"
    desc: " (aura,emotion,fear,visual) 15 feet. Creatures entering or starting their turn in the emanation that can see Ulgrem-Axaan must succeed at a DC 22 Will check save or be [[Conditions/Frightened 1|Frightened 1]] ([[Conditions/Frightened 1|Frightened 2]] on a critical failure). A creature that succeeds at its save is temporarily immune to baleful glare for 1 minute."

  - name: "[[Bestiary Ability Glossary/Ferocity|Ferocity]]"
    desc: "`pf2:r`  "

  - name: "Territorial Rage"
    desc: "`pf2:r`  **Trigger** A creature comes within melee reach of Ulgrem-Axaan while it's burrowed underground or underwater\n**Effect** Ulgrem-Axaan lunges and makes a jaws Strike against the creature, which is [[Conditions/Off-Guard|Off-Guard]] against the attack and takes an additional 1d6 damage on a hit."

attacks:
  - name: ""

  - name: "Melee"
    desc: "`pf2:1` Jaws +18 (reach 10 feet)\n__Damage__  2d12 + 8 piercing plus grab"

  - name: "Melee"
    desc: "`pf2:1` Tail +18 (agile, reach 10 feet)\n__Damage__  2d8 + 8 bludgeoning plus grab"

  - name: "[[Bestiary Ability Glossary/Aquatic Ambush|Aquatic Ambush]]"
    desc: "`pf2:1`  30 feet"

  - name: "[[Bestiary Ability Glossary/Swallow Whole|Swallow Whole]]"
    desc: "`pf2:1` (attack) Medium, 1d12 + 8 bludgeoning, Rupture 18"

  - name: "Unnatural Vengeance"
    desc: "  Ulgrem-Axaan gains a +1 circumstance bonus to attack and damage rolls against undead."

  - name: "Voltaic Blow"
    desc: "`pf2:1` (electricity) **Requirements** Ulgrem-Axaan has a creature [[Conditions/Grabbed|Grabbed]] with its tail\n**Effect** An electric shock passes through Ulgrem-Axaan's tail, dealing 2d4 + 8 electricity damage to the creature (DC 25 Fortitude check save)."

  - name: "[[Bestiary Ability Glossary/Grab|Grab]]"
    desc: "`pf2:1`  "
 
```




