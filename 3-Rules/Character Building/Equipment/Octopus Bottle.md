---
obsidianUIMode: preview
title: "Octopus Bottle"
cssclasses: pf2e,pf2e-spell
tags:
  - trait/alchemical
  - trait/consumable
  - trait/expandable
  - trait/uncommon
  - category/other
aliases: "Compendium.pf2e.equipment-srd.Item.etgPdPhPviKkaFO8"
license: OGL
---
# Octopus Bottle
## Item 8
### [uncommon](uncommon "Uncommon Rarity Trait")[[alchemical]][[consumable]][[expandable]]

**Category** other; 
**Price** 100 gp; 
**Bulk** L; **Size** med
**Usage** held-in-one-hand

**Activate** `pf2:2` Interact

* * *

Miniature octopus arms press up against the sides of this bottle, obscuring the rest of its contents. When opened, a Huge octopus bursts forth, which can appear in water instead of on the ground. Its arms attempt to grasp a creature with a reach of 15 feet. The octopus repositions that creature to a different space within its reach unless the target succeeds at a DC 24 Fortitude check save.

If the octopus is in water, it then releases a cloud of ink in a @Template\[emanation|distance:30\]. This cloud has no effect outside of water. Creatures inside the cloud are undetected and can't use their sense of smell. The cloud dissipates after 1 minute.

* * *

**Craft Requirements** Supply a giant octopus corpse.

*Source: Pathfinder Treasure Vault*
*OGL*